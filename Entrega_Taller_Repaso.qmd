...
Seguidamente, cuantificaremos la magnitud del impacto del cambio climático en la frecuencia de fenómenos extremos, donde se: 
- Calcula el grado en el que el aumento de la temperatura media global está asociado con el incremento de fenómenos extremos.
...
Ahora vamos a ver, que tipo de evntos era más frequente, por cada periodo de tiempo.
...
Finalmente, calcularmos la probabilidad de un evento de interés, por ejemplo "Inundación", donde dicha muestra es de tamaño 20.
...
```
Ahora, vamos a visualizar el histograma para la distancia de Mahalanobis original y para la distancia de Mahalanobis transformada.
...
```
Como vemos, la distancia de Mahalanobis original si que parece que sigue una distribución $\chi^2_6$, pero en cambio la transformada no. Hagamos la prueba estadística para confirmar distribución $\chi^2_6$.
...
Como el $p-value<0.05$, entonces tenemos que efectivamente sigue una $\chi^2_6$.
